...
public sealed class DragAndDropScanner
{
    ...
    private readonly string _tmpFolder = Path.Combine(Path.GetTempPath(), "UMManager_TMP");
    ...
    {
        _workFolder = Path.Combine(_tmpFolder, Guid.NewGuid().ToString("N"));
    ...
    {
        Bundled7Zip, // 7zip bundled with UMManager
