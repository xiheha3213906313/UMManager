...
public class SkinMod : Mod, ISkinMod
{
    ...
    public bool ContainsOnlyJasmFiles()
    {
        return _modDirectory.EnumerateFiles()
            .All(file =>
                file.Name.StartsWith(Constants.InternalFilePrefix, StringComparison.CurrentCultureIgnoreCase) ||
                file.Name.StartsWith(Constants.LegacyInternalFilePrefix, StringComparison.CurrentCultureIgnoreCase));
    }
    ...
    public async Task<string?> GetModIniPathAsync()
    {
        ...
        // Or that UMManager should not touch the merged.ini file.
