...
using UMManager.Core.Entities.Mods.FileModels;
...
namespace UMManager.Core.Entities.Mods.SkinMod;
...
public class SkinModSettingsManager
{
    ...
    private readonly string legacyConfigFileName = Constants.LegacyModConfigFileName;
    private const string CoverImageName = ".UMManager_Cover";
    private const string LegacyCoverImageName = ".JASM_Cover";

    private string _settingsFilePath => Path.Combine(_skinMod.FullPath, configFileName);
    private string _legacySettingsFilePath => Path.Combine(_skinMod.FullPath, legacyConfigFileName);
    ...
    internal async Task<Guid> InitializeAsync()
    {
        ...
        var settingsFilePath = File.Exists(_settingsFilePath)
            ? _settingsFilePath
            : _legacySettingsFilePath;

        if (File.Exists(settingsFilePath))
        {
            var modSettings = await ReadSettingsFromPathAsync(settingsFilePath).ConfigureAwait(false);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (!File.Exists(_settingsFilePath) && File.Exists(_legacySettingsFilePath) && _settings is not null)
            {
                var jsonSkinSettings = _settings.ToJsonSkinSettings(_skinMod);
                await SaveSettingsAsync(jsonSkinSettings).ConfigureAwait(false);
            }
        }
    ...
    private async Task<ModSettings> ReadSettingsFromPathAsync(string fullPath, CancellationToken cancellationToken = default)
    {
        var stream = File.OpenRead(fullPath);
        await using (stream.ConfigureAwait(false))
        {
            return await InternalReadSettingsAsync(_skinMod, stream, cancellationToken).ConfigureAwait(false);
        }
    }
    ...
    private async Task CopyAndSetModImage(ModSettings modSettings, Uri imagePath, bool deleteOldImage = true)
    {
        ...
        var newImageFileName = CoverImageName + Path.GetExtension(imagePath.LocalPath);
    ...
    private bool IsJasmImageFile(Uri? imagePath)
    {
        ...
        return (Path.GetFileName(fsPath).StartsWith(CoverImageName, StringComparison.OrdinalIgnoreCase) ||
                Path.GetFileName(fsPath).StartsWith(LegacyCoverImageName, StringComparison.OrdinalIgnoreCase))
