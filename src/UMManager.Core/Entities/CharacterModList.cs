...
namespace UMManager.Core.Entities;

public sealed class CharacterModList : ICharacterModList
{
    private readonly ILogger? _logger;
    private readonly string _characterFolderName;
    ...
    internal CharacterModList(IModdableObject character, string absPath, ILogger? logger = null)
    {
        ...
        _characterFolderName = new DirectoryInfo(absPath).Name;
        ...
        _selfWatcher.Renamed += OnSelfFolderRenamed;
    ...
    {
        if (!_characterFolderName.Equals(e.Name, StringComparison.OrdinalIgnoreCase))
            return;
    ...
    private void OnSelfFolderRenamed(object sender, RenamedEventArgs e)
    {
        var oldName = e.OldName;
        var newName = e.Name;

        if (oldName is not null && _characterFolderName.Equals(oldName, StringComparison.OrdinalIgnoreCase))
        {
            if (_watcher is null)
                throw new InvalidOperationException("Self watcher is null when mod folder is renamed away");
        ...
        if (newName is null || !_characterFolderName.Equals(newName, StringComparison.OrdinalIgnoreCase))
            return;

        if (_watcher is not null)
            throw new InvalidOperationException("Watcher is not null when mod folder is renamed");
    ...
    private bool IsModFolderEventPath(string? fullPath)
    {
        if (fullPath.IsNullOrEmpty())
            return false;

        var relative = Path.GetRelativePath(AbsModsFolderPath, fullPath);
    ...
    private static string GetModParentFolderPath(ISkinMod mod)
    {
        if (!Path.IsPathFullyQualified(mod.OnlyPath))
            throw new InvalidOperationException("Mod.OnlyPath is not a fully qualified path");
        return mod.OnlyPath;
    }

    private static bool FolderAlreadyExistsIn(string absFolderPath, string folderName)
    {
        ...
        var enabledFolderNamePath = Path.Combine(absFolderPath,
            ModFolderHelpers.GetFolderNameWithoutDisabledPrefix(folderName));
        var disabledFolderNamePath =
            Path.Combine(absFolderPath, ModFolderHelpers.GetFolderNameWithDisabledPrefix(folderName));
    ...
    {
        if (!IsModFolderEventPath(e.FullPath))
            return;
    ...
    {
        if (!IsModFolderEventPath(e.OldFullPath) && !IsModFolderEventPath(e.FullPath))
            return;
    ...
    public void EnableMod(Guid modId)
    {
        try
        {
            ...
            var modParentFolder = GetModParentFolderPath(mod);
            if (Directory.Exists(Path.Combine(modParentFolder, newName)))
    ...
    public void RenameMod(ISkinMod mod, string newName)
    {
        lock (_modsLock)
        {
            ...
            var modParentFolder = GetModParentFolderPath(mod);
            if (FolderAlreadyExistsIn(modParentFolder, newName))
    ...
    public bool FolderAlreadyExists(string folderName)
    {
        ...
        var characterFolder = new DirectoryInfo(AbsModsFolderPath);
        ...
        {
            if (FolderAlreadyExistsIn(skinFolder.FullName, folderName))
                return true;
        }
