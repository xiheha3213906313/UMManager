...
{
    private const string SolutionFile = "UMManager.sln";
    ...
    public static DirectoryInfo GetGamesFolder(string game)
    {
        ...
        var assetsFolder = new DirectoryInfo(Path.Combine(devProjectFolder.FullName, $"UMManager.WinUI\\Assets\\Games\\{game}"));
    ...
    {
        var tmpFolder = new DirectoryInfo(Path.Combine(Path.GetTempPath(), "UMManager_TMP", Guid.NewGuid().ToString()));
        if (!tmpFolder.Exists)
            tmpFolder.Create();
        return tmpFolder;
    }
