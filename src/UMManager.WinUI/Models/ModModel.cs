...
namespace UMManager.WinUI.Models;
    ...
    private Task ToggleModAsync()
    {
        if (_toggleMod is not null)
            ...
            catch (Exception e)
            {
                ...
                var title = IsEnabled
                    ? localizer.GetLocalizedStringOrDefault("Notification.ModToggle.DisableError.Title",
                        defaultValue: "禁用模组时发生错误：{0}")
                    : localizer.GetLocalizedStringOrDefault("Notification.ModToggle.EnableError.Title",
                        defaultValue: "启用模组时发生错误：{0}");
                App.GetService<NotificationManager>().ShowNotification(
                    string.Format(CultureInfo.CurrentUICulture, title!, Name),
                    e.ToString(),
                    null);
            }
