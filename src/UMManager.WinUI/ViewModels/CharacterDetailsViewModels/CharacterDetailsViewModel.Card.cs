...
public partial class CharacterDetailsViewModel
{
    ...
    private void RefreshSelectableInGameSkins()
    {
        SelectableInGameSkins.Clear();

        if (Character is null)
            return;

        foreach (var characterInGameSkin in Character.Skins)
        {
        ...
        foreach (var selectableInGameSkin in SelectableInGameSkins)
            selectableInGameSkin.IsSelected = selectableInGameSkin.InternalName.Equals(SelectedSkin.InternalName.Id,
        ...
        await CommandWrapperAsync(true, async () =>
        {
            ...
            if (characterSkin is null)
            {
                ...
                _notificationService.ShowNotification(
                    _localizer.GetLocalizedStringOrDefault("Notification.SwitchCharacterSkinError.Title",
                        defaultValue: "切换角色皮肤失败"),
                    "",
    ...
    private async Task DisableAllModsAsync()
    {
        ...
        {
            _notificationService.ShowNotification(
                _localizer.GetLocalizedStringOrDefault("Notification.DisableAllModsError.Title", defaultValue: "禁用模组失败"),
                _localizer.GetLocalizedStringOrDefault("Notification.DisableAllModsError.Message", defaultValue: "无法禁用全部模组。"),
                TimeSpan.FromSeconds(5));
            return;
        }
