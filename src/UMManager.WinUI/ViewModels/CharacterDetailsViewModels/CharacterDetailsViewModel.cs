...
public partial class CharacterDetailsViewModel : ObservableObject, INavigationAware
{
    ...
    [ObservableProperty] private string _loadingItemText = "角色";
        ...
        Task.Run(async () =>
        {
            ...
            if (exception is not null)
                _notificationService.ShowNotification(
                    _localizer.GetLocalizedStringOrDefault("Notification.CharacterDetailsLoadError.Title",
                        defaultValue: "加载角色详情页时发生错误。"),
                    exception.Message,
                    null);
            else
                _notificationService.ShowNotification(
    ...
    private async Task CommandWrapperAsync(bool startHardBusy, Func<Task> command)
    {
        ...
        {
            _notificationService.ShowNotification(
                _localizer.GetLocalizedStringOrDefault("Notification.UnknownCommandError.Title",
                    defaultValue: "执行命令时发生未知错误"),
                e.Message,
