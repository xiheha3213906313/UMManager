        ...
        await foreach (var loadModMessage in _modRefreshChannel.Reader.ReadAllAsync(CancellationToken.None))
        {
            ...
            catch (Exception e)
            {
                _notificationService.ShowNotification(
                    _localizer.GetLocalizedStringOrDefault("Notification.ErrorRefreshingMods.Title", defaultValue: "刷新模组失败"),
                    e.Message,
                    null);
            }
        }
        ...
        await Task.Run(async () =>
        {
            ...
            if (refreshResult.ModsDuplicate.Any())
            {
                var message = string.Format(
                    _localizer.GetLocalizedStringOrDefault("Notification.DuplicateModsDetected.MessageHeader",
                        defaultValue: "在 {0} 的模组文件夹中检测到重复模组：\n")!,
                ...
                message = refreshResult.ModsDuplicate.Aggregate(message,
                    (current, duplicateMod) =>
                        current +
                        string.Format(
                            _localizer.GetLocalizedStringOrDefault("Notification.DuplicateModsDetected.MessageItem",
                                defaultValue: "为避免冲突，模组“{0}”已重命名为“{1}”。\n")!,
                            duplicateMod.ExistingFolderName,
                            duplicateMod.RenamedFolderName));

                _notificationService.ShowNotification(
                    _localizer.GetLocalizedStringOrDefault("Notification.DuplicateModsDetected.Title",
                        defaultValue: "检测到重复模组"),
                    message,
    ...
    private async Task ToggleModAsync(ModRowVM? modVmToToggle)
    {
        ...
        try
        {
            await Task.Run(() =>
            {
                ...
                {
                    _notificationService.ShowNotification(
                        _localizer.GetLocalizedStringOrDefault("Notification.DisableModError.Title", defaultValue: "禁用模组时出错"),
                        e.Message,
        ...
        {
            _notificationService.ShowNotification(
                _localizer.GetLocalizedStringOrDefault("Notification.ToggleModError.Title", defaultValue: "切换模组状态时出错"),
                e.Message,
                TimeSpan.FromSeconds(5));
        }
    ...
    private async Task UpdateModSettingsAsync(UpdateModSettingsArgument? arg)
    {
        ...
        if (result.IsError)
        {
            if (result.HasNotification)
                ...
                _notificationService.ShowNotification(
                    _localizer.GetLocalizedStringOrDefault("Notification.SaveModSettingsError.Title",
                        defaultValue: "保存模组设置时出错"),
                    result.Exception?.ToString() ?? result.ErrorMessage ?? "",
        ...
        {
            _notificationService.ShowNotification(
                _localizer.GetLocalizedStringOrDefault("Notification.ModSettingsSaved.Title", defaultValue: "模组设置已保存"),
                messageBody,
