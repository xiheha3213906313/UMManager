    ...
    : ObservableRecipient, IRecipient<ModChangedMessage>
{
        ...
        await foreach (var loadModMessage in _channel.Reader.ReadAllAsync(CancellationToken.None))
        {
            ...
            catch (Exception e)
            {
                ...
                _notificationService.ShowNotification(
                    localizer.GetLocalizedStringOrDefault("Notification.ModPane.LoadFailed.Title", defaultValue: "加载模组失败"),
                    e.Message,
                    null);
            }
        }
            ...
            catch (Exception e)
            {
                ...
                _notificationService.ShowNotification(
                    string.Format(CultureInfo.CurrentUICulture,
                        localizer.GetLocalizedStringOrDefault("Notification.KeySwap.LoadFailed.Title",
                            defaultValue: "加载热键失败：{0}")!,
                        mod.GetDisplayName()),
                    e.Message,
                    null);
            }
    ...
    private async Task SetModIniFileAsync()
    {
        ...
        try
        {
            ...
            var localizer = App.GetService<ILanguageLocalizer>();
            _notificationService.ShowNotification(
                localizer.GetLocalizedStringOrDefault("Notification.ModPane.CopiedFolderPath.Title",
                    defaultValue: "已复制模组文件夹路径到剪贴板"),
                string.Empty,
        ...
        filePicker.CommitButtonText = App.GetService<ILanguageLocalizer>()
            .GetLocalizedStringOrDefault("Common.Button.Set", defaultValue: "设置");
    ...
    private async Task PickImageUriAsync()
    {
        ...
        var filePicker = new FileOpenPicker();
        filePicker.CommitButtonText = App.GetService<ILanguageLocalizer>()
            .GetLocalizedStringOrDefault("Common.Button.SetImage", defaultValue: "设置图片");
    ...
    private async Task PasteImageFromClipboardAsync()
    {
        await CommandWrapper(async () =>
        {
            ...
            if (!clipboardHasValidImageResult.Result)
            {
                var localizer = App.GetService<ILanguageLocalizer>();
                _notificationService.ShowNotification(
                    localizer.GetLocalizedStringOrDefault("Notification.Clipboard.NoValidImage.Title",
                        defaultValue: "剪贴板不包含有效图片"),
                    string.Empty,
                    null);
                return;
            }
            ...
            if (imagePath == null)
            {
                var localizer = App.GetService<ILanguageLocalizer>();
                _notificationService.ShowNotification(
                    localizer.GetLocalizedStringOrDefault("Notification.Clipboard.GetImageFailed.Title",
                        defaultValue: "无法从剪贴板获取图片"),
                    string.Empty,
                    null);
                return;
            }
    ...
    private async Task SaveModSettingsAsync()
    {
        await CommandWrapper(async () =>
        {
            ...
            if (savingKeySwapException is not null)
            {
                var localizer = App.GetService<ILanguageLocalizer>();
                _notificationService.ShowNotification(
                    localizer.GetLocalizedStringOrDefault("Notification.KeySwap.SaveFailed.Title",
                        defaultValue: "保存热键配置失败"),
                    savingKeySwapException.Message,
                    null);
            }
        ...
        bool useDefaultExceptionHandler = false, [CallerMemberName] string commandName = "")
    {
        ...
        catch (Exception e)
        {
            if (useDefaultExceptionHandler)
            {
                ...
                _notificationService.ShowNotification(
                    string.Format(CultureInfo.CurrentUICulture,
                        localizer.GetLocalizedStringOrDefault("Notification.Command.RunFailed.Title",
                            defaultValue: "运行命令时发生错误：{0}")!,
                        commandName),
                    e.Message,
                    null);
                return;
