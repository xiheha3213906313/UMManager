...
using Serilog;
using Constants = UMManager.Core.Helpers.Constants;
using static UMManager.WinUI.ViewModels.CloseRequestedArgs;
...
public partial class ModInstallerVM : ObservableRecipient, INavigationAware, IDisposable
{
        ...
        DispatcherQueue dispatcherQueue, ICharacterSkin? inGameSkin = null, InstallOptions? options = null)
    {
        ...
        _modInstallation = ModInstallation.Start(modToInstall, _characterModList, inGameSkin);
    ...
    private void SuccessfulInstall(ISkinMod newMod)
    {
        ...
        _notificationManager.ShowNotification(
            string.Format(CultureInfo.CurrentUICulture,
                localizer.GetLocalizedStringOrDefault("Notification.ModInstaller.Installed.Title",
                    defaultValue: "模组“{0}”已安装")!,
                modName),
            string.Format(CultureInfo.CurrentUICulture,
                localizer.GetLocalizedStringOrDefault("Notification.ModInstaller.Installed.Message",
                    defaultValue: "模组“{0}”（{1}）已成功添加到 {2} 模组列表")!,
                modName,
                newMod.Name,
                _characterModList.Character.DisplayName),
        ...
        App.MainWindow.DispatcherQueue.EnqueueAsync(() =>
            _modNotificationManager.AddModNotification(new ModNotification()
            {
                ...
                Message = localizer.GetLocalizedStringOrDefault("Notification.ModInstaller.Added.ShortMessage",
                    defaultValue: "模组已成功添加")!
            }));
    }
    ...
    private async Task PasteModImageAsync()
    {
        ...
        catch (Exception e)
        {
            ...
            _notificationManager.ShowNotification(
                localizer.GetLocalizedStringOrDefault("Notification.RetrieveImageFromClipboardFailed.Title",
                    defaultValue: "从剪贴板获取图片失败"),
                e.Message,
                TimeSpan.FromSeconds(5));
            return;
        }
    ...
    private bool canAddModAndRename()
    {
        ...
        if (!ModFolderHelpers.FolderNameEquals(DuplicateModFolderName, _duplicateMod.Name))
            if (_modInstallation.FolderNameExistsInTargetSkin(DuplicateModFolderName, excludeModId: _duplicateMod.Id))
                return false;

        if (!ModFolderHelpers.FolderNameEquals(ModFolderName, _modInstallation.ModFolder.Name))
            if (_modInstallation.FolderNameExistsInTargetSkin(ModFolderName))
                return false;
        ...
        try
        {
            ...
            if (ModPreviewImagePath == _placeholderImageUri || overrideCurrent)
            {
                ...
                catch (Exception e)
                {
                    ...
                    var localizer = App.GetService<ILanguageLocalizer>();
                    _notificationManager.ShowNotification(
                        localizer.GetLocalizedStringOrDefault("Notification.DownloadImageFailed.Title",
                            defaultValue: "从模组链接下载图片失败"),
                        e.Message,
                        TimeSpan.FromSeconds(5));
                }
            }
    ...
    private void ErrorOccurred(Exception e)
    {
        ...
        var localizer = App.GetService<ILanguageLocalizer>();
        _notificationManager.ShowNotification(
            localizer.GetLocalizedStringOrDefault("Notification.ModInstallerError.Title", defaultValue: "发生错误"),
            localizer.GetLocalizedStringOrDefault("Notification.ModInstallerError.Message",
                defaultValue: "添加模组时发生错误，详情请查看日志。"),
        ...
        if (_inGameSkin is null)
        {
            ...
            else
            {
                ...
                _notificationManager.QueueNotification(
                    localizer.GetLocalizedStringOrDefault("Notification.ModInstaller.SkinUndetected.Title",
                        defaultValue: "无法识别新模组的皮肤"),
                    localizer.GetLocalizedStringOrDefault("Notification.ModInstaller.SkinUndetected.Message",
                        defaultValue: "UMManager 无法确定该模组对应的游戏内皮肤，因此无法判断应禁用哪些模组。"));
                return;
            }
        }
