...
public partial class CreateCommandViewModel : ObservableObject
{
    ...
    private async Task SelectExecutableAsync()
    {
        ...
        var filePicker = new FileOpenPicker
        {
            ...
            CommitButtonText = localizer.GetLocalizedStringOrDefault("Common.Button.Select", defaultValue: "选择")
        };
    ...
    private async Task SelectWorkingDirectoryAsync()
    {
        ...
        {
            SuggestedStartLocation = PickerLocationId.ComputerFolder,
            CommitButtonText = localizer.GetLocalizedStringOrDefault("Common.Button.SelectFolder", defaultValue: "选择文件夹")
        };
    ...
    private async Task CreateCommandAsync()
    {
        ...
        catch (Exception e)
        {
            ...
            _notificationManager.ShowNotification(
                IsEditingCommand
                    ? localizer.GetLocalizedStringOrDefault("Notification.Command.UpdateFailed.Title", defaultValue: "更新命令失败")
                    : localizer.GetLocalizedStringOrDefault("Notification.Command.CreateFailed.Title", defaultValue: "创建命令失败"),
                e.Message,
    ...
    private async Task InternalCreateCommand()
    {
        ...
        if (_createOptions?.IsEditingCommand == true)
        {
            ...
            _notificationManager.ShowNotification(
                string.Format(CultureInfo.CurrentUICulture,
                    localizer.GetLocalizedStringOrDefault("Notification.Command.Updated.Title",
                        defaultValue: "命令“{0}”已更新")!,
                    createOptions.CommandDisplayName),
                string.Empty,
        ...
        else
        {
            ...
            _notificationManager.ShowNotification(
                string.Format(CultureInfo.CurrentUICulture,
                    localizer.GetLocalizedStringOrDefault("Notification.Command.Created.Title",
                        defaultValue: "命令“{0}”已创建")!,
                    createOptions.CommandDisplayName),
                string.Empty,
                TimeSpan.FromSeconds(3));
