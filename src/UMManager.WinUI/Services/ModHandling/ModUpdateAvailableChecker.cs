...
using Serilog;
using static UMManager.WinUI.Services.ModHandling.ModUpdateAvailableChecker;
    ...
    private async Task CatchAll(Func<Task> func, string methodName)
    {
        ...
        catch (Exception e)
        {
            ...
            _notificationManager.ShowNotification(
                ...
                $"A fatal error occured in the background checker ({methodName} : {e.HResult}). This means that UMManager can no longer check for mod updates in the background or manually. Error: {e}",
    ...
    private async Task RunCheckerAsync(ModCheckOperation modCheckOperation, CancellationToken runningCancellationToken)
    {
        ...
        if (!modCheckOperation.ModsToCheck.Any())
        {
            if (!modCheckOperation.ModCheckRequest.ScheduledCheck)
                _notificationManager.ShowNotification(
                    localizer.GetLocalizedStringOrDefault("Notification.NoModsToCheckUpdates.Title",
                        defaultValue: "没有可检查更新的模组"),
                    localizer.GetLocalizedStringOrDefault("Notification.NoModsToCheckUpdates.Message",
                        defaultValue: "没有任何模组满足条件，因此未执行检查。"),
        ...
        {
            _notificationManager.ShowNotification(
                localizer.GetLocalizedStringOrDefault("Notification.ModUpdateCheckFinished.Title", defaultValue: "模组更新检查完成"),
                string.Format(localizer.GetLocalizedStringOrDefault("Notification.ModUpdateCheckFinished.ForCharacter",
                        defaultValue: "已为 {1} 检查 {0} 个模组的更新。")!,
        ...
        else if (anyModsChecked)
        {
            _notificationManager.ShowNotification(
                localizer.GetLocalizedStringOrDefault("Notification.ModUpdateCheckFinished.Title", defaultValue: "模组更新检查完成"),
                localizer.GetLocalizedStringOrDefault("Notification.ModUpdateCheckFinished.Message",
                    defaultValue: "模组更新检查完成。"),
                TimeSpan.FromSeconds(4));
        }
    }
