    ...
    CommandService commandService)
{
        ...
        Func<Task>? postShutdownLogic = null)
    {
        ...
        if (notifyOnError)
        {
            ...
            _notificationManager.ShowNotification(
                localizer.GetLocalizedStringOrDefault("Notification.RestartFailed.Title", defaultValue: "重启应用失败"),
                useLegacyRestartOnError
                    ? string.Format(CultureInfo.CurrentUICulture,
                        localizer.GetLocalizedStringOrDefault("Notification.RestartFailed.TryLegacy.Message",
                            defaultValue: "将尝试使用兼容方式重启。原因：{0}")!,
                        error)
                    : string.Format(CultureInfo.CurrentUICulture,
                        localizer.GetLocalizedStringOrDefault("Notification.RestartFailed.Manual.Message",
                            defaultValue: "请手动重启。原因：{0}")!,
                        error),
    ...
    public async Task LegacyRestartAsync(string args = "")
    {
        ...
        catch (Exception e)
        {
            ...
            _notificationManager.ShowNotification(
                localizer.GetLocalizedStringOrDefault("Notification.RestartFailed.Title", defaultValue: "重启应用失败"),
                localizer.GetLocalizedStringOrDefault("Notification.RestartFailed.ManualShort.Message",
                    defaultValue: "请手动重启"),
                TimeSpan.FromSeconds(4));
    ...
    {
        _logger.Debug("UMManager starting shutdown cleanup...");
        ...
        _logger.Debug("UMManager shutdown cleanup complete.");
    }
