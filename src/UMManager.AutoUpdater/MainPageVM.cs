...
{
    private readonly string WorkDir = Path.Combine(Path.GetTempPath(), "UMManager_Auto_Updater");
    ...
    private async Task<GitHubRelease?> IsNewerVersionAvailable(CancellationToken cancellationToken)
    {
        ...
        var getJasmAsset =
            newestVersionFound?.assets?.FirstOrDefault(a => a.name?.StartsWith("UMManager_", StringComparison.OrdinalIgnoreCase) ?? false) ??
            newestVersionFound?.assets?.FirstOrDefault(a => a.name?.StartsWith("JASM_", StringComparison.OrdinalIgnoreCase) ?? false);

        if (getJasmAsset?.browser_download_url is null)
        {
            Stop(
                "Could not find the app archive in the newest release on GitHub. This may be due to the developer having to manually upload the zip which can take a few minutes. " +
                "If the problem persists, then you may have to update manually");
            return null;
        }
        ...
        release.FileName = getJasmAsset.name ?? "UMManager.zip";
    ...
    private async Task UnzipLatestVersion(CancellationToken cancellationToken)
    {
        ...
        _extractedJasmFolder =
            new DirectoryInfo(WorkDir).EnumerateDirectories().FirstOrDefault(folder =>
                folder.Name.StartsWith("UMManager", StringComparison.CurrentCultureIgnoreCase)) ??
            new DirectoryInfo(WorkDir).EnumerateDirectories().FirstOrDefault(folder =>
                folder.Name.StartsWith("JASM", StringComparison.CurrentCultureIgnoreCase))!;

        if (_extractedJasmFolder is null)
        {
            Stop("Failed to find UMManager folder in extracted zip file");
            return;
        }

        Log($"UMManager application folder extracted successfully. Path: {_extractedJasmFolder.FullName}");
    }
    ...
    private async Task InstallLatestVersion()
    {
        ...
        {
            Stop("Failed to find installed UMManager folder in path");
            return;
        }

        string[] appExeCandidates = ["UMManager.exe", "JASM - Just Another Skin Manager.exe"];
        const string preferredExe = "UMManager.exe";
        _newJasmExePath = Path.Combine(_installedJasmFolder.FullName, preferredExe);
        ...
        {
            if (appExeCandidates.Any(candidate =>
                    fileSystemInfo.Name.Equals(candidate, StringComparison.CurrentCultureIgnoreCase)))
            {
        ...
        {
            Stop(
                $"Failed to find the app executable in installed folder. Expected one of: {string.Join(", ", appExeCandidates)}. Path: {_installedJasmFolder}");
            return;
        }
        ...
        {
            Stop(
                $"UMManager folder seems to contain windows system files, this should never happen. File Found: '{systemFileFound}' at " +
        ...
        string[] doNotDeleteFiles =
        [
            "Elevator.exe",
            "UMManager.exe.WebView2",
            "JASM - Just Another Skin Manager.exe.WebView2",
            "logs"
        ];
        ...
        var updatedExePath = appExeCandidates
            .Select(exeName => Path.Combine(_installedJasmFolder.FullName, exeName))
            .FirstOrDefault(File.Exists);

        if (updatedExePath is null)
        {
            Stop("Update completed, but could not find the application executable to start.");
            return;
        }

        _newJasmExePath = updatedExePath;

        Log("UMManager updated successfully");
    }
    ...
    // Copied from UMManager.WinUI/Services/UpdateChecker.cs
    ...
    private HttpClient CreateHttpClient()
    {
        ...
        httpClient.DefaultRequestHeaders.Add("User-Agent", "UMManager-Update-Checker");
        return httpClient;
    }
    ...
    private async Task<ContentDialogResult> ShowDeleteWarning(ICollection<string> warningFiles)
    {
        ...
        {
            Text =
                "已安装的 UMManager 文件夹内的所有文件/文件夹都将被永久删除！\n" +
                "（不包含更新器自身的更新文件夹）该操作不可撤销。\n" +
                $"UMManager 目录：{_installedJasmFolder.FullName}",
        ...
        if (warningFiles.Any())
            stackPanel.Children.Add(new TextBlock
            {
                Text = "以下文件/文件夹不属于 UMManager，也会一并删除：\n" +
                       string.Join("\n", warningFiles),
                IsTextSelectionEnabled = true,
                TextWrapping = TextWrapping.WrapWholeWords,
                Margin = new Thickness(0, 0, 0, 10)
